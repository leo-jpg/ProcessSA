    ...
    <body>
        <sql:setDataSource var="dataSource" driver="oracle.jdbc.OracleDriver" url="jdbc:oracle:thin:@localhost:1521:XE" user="TASK" password="admin"></sql:setDataSource>
         <sql:query dataSource="${dataSource}" var="tipo">
            SELECT id_tarea, nombre, descripcion, plazo, cumplimiento, id_usuario_asignado, proceso_id_proceso, estado_tarea_id_estado, indicador_id_indicador FROM tarea WHERE usuario_id_usuario = ${id} AND estado_tarea_id_estado=2
        </sql:query> 
                ...
                <c:forEach var="aux" items="${tipo.rows}">
                    <tbody>
                        <tr>
                            ...
                            <td><a href="reasignarTarea.jsp">Reasignar</a></td>
                        </tr>
                    </tbody>
                </c:forEach>
        </table>
